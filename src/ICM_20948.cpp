...
static void transfer(const struct spi_dt_spec* spi, uint8_t* tx_buf, size_t len, uint8_t* rx_buf)
{
    __ASSERT(tx_buf != NULL, "spi tx buff is NULL!");
    __ASSERT(rx_buf != NULL, "spi rx buff is NULL!");

    int err;
    uint8_t dummy_rx_buf[len];
    struct spi_buf rx_spi_buf;
    if (rx_buf != NULL)
    {
        rx_spi_buf.buf = (void*)rx_buf;
    }
    else
    {
        rx_spi_buf.buf = (void*)dummy_rx_buf;
    }
    rx_spi_buf.len = len;
...
ICM_20948_Status_e ICM_20948::startupMagnetometer(bool minimal)
{
    ...
    if (tries == MAX_MAGNETOMETER_STARTS)
    {
        ...
        debugPrintf(MAX_MAGNETOMETER_STARTS);
...
ICM_20948_Status_e ICM_20948_SPI::begin(const struct spi_dt_spec* spi, const struct gpio_dt_spec* intterupt, uint32_t SPIFreq)
{
    __ASSERT(spi_is_ready(spi), "spi device is not ready!");
    ...
    uint8_t kick[] = { 0x00 };
    uint8_t dummy[] = { 0x00 };
    transfer(_spi, kick, sizeof(kick), dummy);
...
ICM_20948_Status_e ICM_20948_write_SPI(uint8_t reg, uint8_t* data, uint32_t len, void* user)
{
    ...
    auto* _spi = ((ICM_20948_SPI*)user)->_spi; // Cast user field to ICM_20948_SPI type and extract the SPI interface pointer
    if (_spi == NULL)
    {
        return ICM_20948_Stat_ParamErr;
    }
    ...
    // uint8_t kick[] = {0x00};
    // transfer(_spi, kick, sizeof(kick));

    uint8_t tx[len + 1] = { 0 };
    uint8_t rx[len + 1] = { 0 };
    tx[0] = (reg & 0x7F) | 0x00;
    memcpy(&tx[1], data, len);
    transfer(_spi, tx, len + 1, rx);
...
ICM_20948_Status_e ICM_20948_read_SPI(uint8_t reg, uint8_t* buff, uint32_t len, void* user)
{
    ...
    auto _spi = ((ICM_20948_SPI*)user)->_spi;
    if (_spi == NULL)
    {
        return ICM_20948_Stat_ParamErr;
    }
